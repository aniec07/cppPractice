...
int main(int argc, char *argv[]) {
	ifstream FIFO;
	string Order = "";
    TradeList SellListAll, BuyListAll;
	while (Order != "x@x@EXCHCLOSED@x@x@x") {
		...
		while (FIFO >> Order){
            time_t CT;
            time(&CT);
            cout << "Current Time Is: " << Current_Time(&CT) << endl;
			cout << Order << endl;
            populateList (BuyListAll,SellListAll,Order);
            checkExpiry (BuyListAll,SellListAll);
            //doTransaction (BuyListAll, SellListAll);
        }
...
void PrintScrip(Scrip s){
    cout << " " << s.name << s.rate << s.type << endl;
    }
