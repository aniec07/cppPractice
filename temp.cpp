...
list<fish> updateFish ( list<fish> &fishList, list<shark> &newsharkList){
    ...
    for ( fl = fishList.begin() ; fl != fishList.end() ; fl++ ){
        ...
        for ( ng = neighbours.begin() ; ng != neighbours.end(); ng++ ){  //scanning all the neighbours of current fish
            if( !isFoundFish(fishList,*ng) ){ //is an empty cell
                fish newfish;
                newfish.x = (*ng).x;
                newfish.y = (*ng).y;
            ...
            if ( isFoundFish(fishList,*ng) && !isFoundFish(newfishList, *ng) ){ // is a live fish member not updated
                fish newfish;
                newfish.x = (*ng).x;
                newfish.y = (*ng).y;
                ...
                else { //the fish continues to next generation
                        fish newfish;
                        newfish.x = (*ng).x;
                        newfish.y = (*ng).y;
                        newfishList.push_back(newfish);
